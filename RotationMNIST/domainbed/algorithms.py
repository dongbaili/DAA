...
from domainbed.lib.misc import (
    random_pairs_of_minibatches, ParamDict, MovingAverage, l2_between_dicts, split_meta_train_test, accuracy_batch, trainable_layers
)
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        if hparams["trainable_layers_start"] >= 1:
            # trainable_list = trainable_layers(hparams["trainable_layers_start"])
            for (name, param) in self.featurizer.named_parameters():
                param.requires_grad = False
                # result = re.search("layer(\d+)", name)
                # if result is None or result[0] not in trainable_list:
                    # print(name)
                    # param.requires_grad = False
        ...
        self.weighter = LogisticRegression(max_iter = 300)
        # self.weighter = RandomForestClassifier()

    def ERM_update(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x,y in minibatches])
        all_y = torch.cat([y for x,y in minibatches])

        y_output = self.predict(all_x)
        loss = F.cross_entropy(y_output, all_y, reduction='mean')
    ...
    def select_pseudo(self, loader, bar, device):
        loader = [x.to(device) for x,y in loader]
        unlabel_x = torch.cat((loader[0], loader[1], loader[2], loader[3][:4]), axis = 0) # 100 unlabel_x
        # print(unlabel_x.shape)
        Y = self.predict(unlabel_x)
        pseudo_x, pseudo_label = [], []
        for y,x in zip(Y, unlabel_x):
            max_value, max_index = torch.max(y, dim=0)
            max_p = torch.max(F.softmax(y, dim = 0))
            if max_p > bar:
                pseudo_x.append(x)
                pseudo_label.append(max_index.item())

        print(len(pseudo_label))
        if len(pseudo_x) == 0:
            return None, None
        else:
            return torch.stack(pseudo_x).to(device), torch.tensor(pseudo_label).to(device)
    
    def SelfSupervise(self, minibatches, pseudo_x, pseudo_y, alpha, device, unlabeled=None):
        all_x = torch.cat([x for x,y in minibatches])
        all_y = torch.cat([y for x,y in minibatches])
        ...
        if pseudo_x is not None:
            # print(pseudo_x.shape, pseudo_y.shape)
            loss_pseudo = F.cross_entropy(self.predict(pseudo_x), pseudo_y, reduction='mean')
            loss += alpha * loss_pseudo
            # print(loss_pseudo)
    ...
    def class_weight(self, y_s, y_pseudo_t):
        weight_s = torch.tensor([torch.sum(y_s == i).item() for i in range(10)])

        weight_t = torch.tensor([torch.sum(y_pseudo_t == i).item() for i in range(10)])

        alpha = weight_t / weight_s
        # print(weight_s, weight_t, alpha)

        weight = torch.tensor([alpha[x] for x in y_s])

        return weight
    
    def WDAN_update(self, minibatches, test_loader, device, gamma = 0.1, lambdda = 0.1):
        test_loader = [x.to(device) for x,y in test_loader]
        x_target = torch.cat((test_loader[0], test_loader[1], test_loader[2], test_loader[3][:4]), axis = 0) # 100 unlabeled test x
        all_x = torch.cat([x for x,y in minibatches])
        all_y = torch.cat([y for x,y in minibatches])

        # loss on source data
        loss_source = F.cross_entropy(self.predict(all_x), all_y, reduction='mean')
        
        # loss on sudo-target data
        y_p_target = self.predict(x_target)

        y_pseudo = torch.argmax(y_p_target, dim = 1)
        loss_sudo_target = F.cross_entropy(y_p_target, y_pseudo, reduction='mean')
        
        # loss on weighted MMD (featurized layer)
        weight = self.class_weight(all_y, y_pseudo)
        mmd = MMD()
        loss_MMD = mmd(self.featurize(all_x), self.featurize(x_target), weight)

        loss = loss_source + gamma * loss_sudo_target + lambdda + loss_MMD
    ...
    def Reweight_update(self, minibatches, test_loaders, device, unlabeled=None):
        all_x = torch.cat([x for x,y in minibatches])
        all_y = torch.cat([y for x,y in minibatches])

        weight = self.UDAWeightPixel(all_x, test_loaders[0], device).detach()
    ...
    def UDAWeightPixel(self, x, loader, device):
        loader = [x.to(device) for x,y in loader]
        x_t = torch.cat((loader[0], loader[1]), axis = 0)
        x_t = x_t.view(x_t.shape[0], -1).cpu().detach().numpy()
        x_s = x.view(x.shape[0], -1).cpu().detach().numpy()

        x = np.concatenate((x_t, x_s), axis = 0)
        y = np.concatenate((np.ones(len(x_t)), np.zeros(len(x_s))), axis = 0)
        model = LogisticRegression(max_iter=300)

        model.fit(x, y)
        p = model.predict_proba(x_s)
...
class MixStyle(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        if self.hparams["ms_type"] == "random":
            all_x = torch.cat([x for x,y in minibatches])
            all_y = torch.cat([y for x,y in minibatches])
            loss = F.cross_entropy(self.predict(all_x), all_y)
        elif self.hparams["ms_type"] == "crossdomain":
            loss = 0
            for (xi, yi), (xj, yj) in random_pairs_of_minibatches(minibatches):
                x_m = torch.cat([xi, xj])
                y_m = torch.cat([yi, yj])
                loss += F.cross_entropy(self.predict(x_m), y_m)
